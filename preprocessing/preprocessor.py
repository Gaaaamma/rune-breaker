...
def standardize_stretch(
        output_size: int,
        filename: str,
        image,
        output_dir: str = SETTINGS.standard_data_dir
    ) -> str:
    """Standardize image by stretching"""
    ...
    # Save image
    logger.info(f"standardized image shape: {resized_image.shape}")
...
def cut_arrows(file: str, debug: bool):
    ...
    for pos in arrows:
        ...
        if not debug:
            ...
            standardize_stretch(SETTINGS.train_img_size, filename, arrow)
...
def cut_arrows_inference(file_path: str, output_dir: str = SETTINGS.inference_tmp):
    """Use blue calculator to determine how to cut"""
    ...
    threshold: int = SETTINGS.laplace_blue_threshold
    toleration: int = SETTINGS.laplace_blue_tolerantion
    statistic_hz: List[int] = blue_calculator_hz(file_path)
    statistic_hz = statistic_hz[SETTINGS.laplace_blue_height_start:SETTINGS.laplace_blue_height_end]
    maximum_score: int = 0
    ...
    img_right: int = 0
    statistic_vt: List[int] = blue_calculator_vt(
        file_path, vt_cut_top, vt_cut_btm,
        SETTINGS.laplace_blue_width_start, SETTINGS.laplace_blue_width_end
    )
    maximum_score = 0
    for col, _ in enumerate(statistic_vt): # 0 ~ len(statisc_vt)
    ...
    file_path = f"{SETTINGS.upload_path}{file_path}"
    raw = cv.imread(file_path)
    # cv.imshow(file_path, raw)
    img = raw[img_top:img_btm, img_left:img_right]
    need_cut = img
...
if __name__ == "__main__":
    ...
    # Cut 4 arrows, standardize and store it
    for file in files:
        logger.info(f"File: {file}")
        cut_arrows(file, SETTINGS.debug)

    # Prepare trained data
    files = listdir(SETTINGS.standard_data_dir)
    files = sorted(files)